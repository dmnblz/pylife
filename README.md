...
* **Events Editor** – Click the Events button to open a centered modal for composing rules programmatically. Choose a Trigger (Sensor enter/stay/exit, Key down/hold/up, Timer every/after) and add ordered blocks: Channel Set, Channel Pulse (duration), Wait (ms), Hold and Release. You can enable/disable rules without deleting them from the "Existing Rules" list. Scenes save these rules under `events` with `events_version` (v2 when sequences or new blocks are used) and persist a per‑rule `enabled` flag.

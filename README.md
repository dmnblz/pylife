...
A **Select** tool can drag out a rectangle to capture multiple particles, springs and bending springs for group operations. Selected entities may be moved together, deleted, have current parameters applied in bulk or copied and pasted elsewhere. Selections can also be saved to disk and later merged into other scenes without replacing existing content, and any pasted or loaded subset can be reverted with the **Undo** button.
Number keys **1–0** switch between the first ten sidebar tools in order and each button displays its shortcut. Internally, the builder now dispatches mouse and keyboard input through per‑mode handlers which are looked up from a small dictionary, simplifying the event logic. The delete tool now removes springs without crashing, inspecting springs no longer triggers errors, and converting springs between normal and variable types is stable.
...
* **Select** – drag a rectangle to highlight multiple entities. Press **Delete** to remove them, **Ctrl+C**/**Ctrl+V** to copy and paste or **Ctrl+S** to save the subset.
